...
ASSIGN
	...
	next(hb) := case
		state = update & !imroot & hb >= ROOT_TIMEOUT : {0,hb}; 
		state = update & hb < ROOT_TIMEOUT : hb + 1; 
	...
	next(myseq) := case
		myroot = ROOT & id = ROOTID & state = sending & myseq < MAXSEQ : myseq + 1; 
...
for(`i',2,N-1,dnl
	...
	next(bc_myseq) := case
		state = sending : myseq;
		myroot = ROOT & main.decrement_myseq & (bc_myseq >= 1) : bc_myseq - 1; 
		finishedSending : 0;
		TRUE : bc_myseq;
  esac;
...
for(`i',2,N-1,dnl
	...
	next(bc_myseq) := case
		state = sending : myseq;
		main.decrement_myseq & (bc_myseq >= 1) : bc_myseq - 1; 
		finishedSending : 0;
		TRUE : bc_myseq;
  esac;
	next(bc_leftdone) := case
