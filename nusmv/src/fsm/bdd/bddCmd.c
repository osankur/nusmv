...
int CommandCheckRealizable ARGS((int argc, char **argv));
...
void Bdd_Init(void){
  ...
  Cmd_CommandAdd("check_realizable", CommandCheckRealizable, 0, false);
...
int CommandCheckRealizable(int argc, char **argv)
{
  BddFsm_ptr fsm;
	int result;
  ...
  fsm = PropDb_master_get_bdd_fsm(PropPkg_get_prop_database());
	result = BddFsm_check_realizable(fsm);
	if (result){
